...
{
    // what if file is not present, what if error?
    rewind(conn->file);
...
{
    if (argc < 3) die("USAGE: 17 <dbfile> <action> [action params]");
    ...
    */
    ...
    switch (action) {
        ...
        default:
            die("Invalid action: [c]reate, [g]et, [s]et, [d]elete, [l]ist");
    }
